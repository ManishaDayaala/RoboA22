...
MAINFOLDER = r"D:\APPdata_RoboA22"

# Create other paths relative to the main folder
training_file_path = os.path.join(MAINFOLDER, "Training", "Training_RoboA22.xlsx")  # FIXED TRAINING DATA
test_file_path = os.path.join(MAINFOLDER, "24hrData", "Dailydata_RoboA22.xlsx")  # DAILY DATA
excel_file_path = os.path.join(MAINFOLDER, "Breakdownrecords_RoboA22.xlsx")  # Recording excel for BD
folderpath = os.path.join(MAINFOLDER, "TemporaryData")  # Temporary dump files collector
threshold_file_path = os.path.join(MAINFOLDER,"Thresholds_RoboA22.xlsx") #
...
def process_data():
    ...
    assets_list = [
        "Robot Grinding Machine-1 MDE","Robot Grinding Machine-1 MNDE","Robot Grinding Machine-1 Spindle DE",
        "Robot Grinding Machine-1 Spindle NDE","Robot Grinding Machine-2 MDE","Robot Grinding Machine-2 MNDE", "Robot Grinding Machine-2 Spindle DE ",
        "Robot Grinding Machine-2 Spindle NDE ", "Robot Polishing MDE", "Robot Polishing MNDE", 
        "Robot Polishing Spindle-1 DE","Robot Polishing Spindle-2 DE"]
...
def train_ensemble_model(training_file_path, model_folder_path):
    ...
    def preprocess_data(X, y):
        ...
        # Save the scaler
        joblib.dump(scaler, os.path.join(model_folder_path, "scaler_RoboA22.pkl"))
    ...
    joblib.dump(ensemble_model, os.path.join(model_folder_path, "ensemble_RoboA22.pkl"))
...
def predict_ensemble(test_file_path, model_folder_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, "scaler_RoboA22.pkl"))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict(X_test_scaled):
        nn_model = joblib.load(os.path.join(model_folder_path, "ensemble_RoboA22.pkl"))
